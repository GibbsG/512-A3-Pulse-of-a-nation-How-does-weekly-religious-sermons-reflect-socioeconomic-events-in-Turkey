...
TextChart.prototype.draw = function() {
	...
	text.text((d) => {return d + " "}).style('color', (d,i) => {
			for (var i = 0; i < self.settings.keywords.length; i++) {
				if (d.includes(self.settings.keywords[i])) return 'red';
			}
			return 'black';
		})
		.style('font-weight', (d,i) => {
			for (var i = 0; i < self.settings.keywords.length; i++) {
				if (d.includes(self.settings.keywords[i])) return 'bold';
			}
			return 'normal';
		})
