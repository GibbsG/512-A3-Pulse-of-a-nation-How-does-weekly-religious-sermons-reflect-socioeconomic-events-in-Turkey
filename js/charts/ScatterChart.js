...
ScatterChart.prototype.draw = function(resetScale, duration) {
	...
	// draw bubbles
	var circles = self.g.selectAll('.circle').data(self.settings.data, function(d) {
		if (d.y > 0.20){
			return d.id
		}
	})
	...
	circles.enter().append('circle').call(self.circlePositionFunction2)
	self.g.selectAll('.circle').transition().duration(500).call(self.circlePositionFunction)
	//self.g.selectAll('.circle').transition().duration(500).call(self.circlePositionFunction2)
...
ScatterChart.prototype.drawLegend = function() {
	...
	if(d3.keys(self.settings.colorLabels).length != 0 || self.settings.colorVar == 'turkish' || self.settings.colorVar == 'kurdish'|| self.settings.colorVar == 'civilians') self.drawCategoricalLegend()
	else self.drawContinuousLegend()
}
...
ScatterChart.prototype.drawCategoricalLegend = function() {
	...
	var labels = legend.selectAll('g')
					...
					.attr('transform', function(d,i) {
						if(self.settings.colorVar == 'turkish' || self.settings.colorVar == 'kurdish'|| self.settings.colorVar == 'civilians')
							return 'translate(-120,' +(((i+1)*40) ) + ')'
						return 'translate(-10,' +(((i+1)*40) ) + ')'
					})
	
	var legRects = labels.append('circle').call(self.legendCircleFunc)
	var text = labels.append('text').call(self.legendTextFunc)
	// labels.append('text').text(self.settings.colorVar)
}
