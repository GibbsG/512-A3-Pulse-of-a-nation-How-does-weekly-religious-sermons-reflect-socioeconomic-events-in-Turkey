...
Chart.prototype.buildTitle = function() {
	...
	self.titleText = self.title.append('text').attr('id', self.settings.id + '-divtitle-text').style("font-weight", "bold")
}
...
Chart.prototype.buildAxisLabels = function() {
	...
	if(self.settings.hasYLabel == true) {
		...
		self.ytitle = self.ytitleDiv.append('text')
			.text(self.topicsObj[self.settings.yLabel]+' prevalence value (>= 0.20)')
...
Chart.prototype.defineFunctions = function() {
	...
	self.legendCircleFunc = function(leg) {
		leg.attr("r", 5)
			.attr("cx", 5)
			.attr("cy", 5)
			.attr('fill', function(d) {
	...
	self.legendTextFunc = function(txt) { // here
		txt.text(function(d) {
			var limit = Math.floor((self.settings.margin.right -10)/10.5)
			var fullText =   d
			if (fullText == '1')
				fullText =   '>=1 casualties'
			if (fullText == '0')
				fullText =   'No casualties'
			if (fullText == 'title') {
				fullText =   self.settings.colorVar
	...
	self.circlePositionFunction = function(circle) {
		circle
			...
			.style('visibility', function(d) {
				if(self.xScale(d.x)<0 | self.xScale(d.x) > self.settings.plotWidth | self.yScale(d.y)<0 | d.y <= 0.2 | self.yScale(d.y)>self.settings.plotHeight){
					return 'hidden'
				} 
				else return 'visible'
			})
	...
	self.circlePositionFunction2 = function(circle) {
		circle
			.attr('transform', self.zoomTransform)
			.attr('r', function(d) {return self.settings.getRadius(d)})
			.style('fill', function(d) {
