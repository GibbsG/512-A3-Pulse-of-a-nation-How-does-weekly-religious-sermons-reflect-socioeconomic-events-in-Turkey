	...
	switch(chart) {
		case 'scatterChart':
			...
			settings[chart].data = self.settings.data.map(function(d, i) {
				...
				return {x:xVal, y:yVal, id:id, text:[d.turkish, d.kurdish, d.civilians], radiusValue:d[self.settings.radiusVar], colorValue:d[self.settings.colorVar]}
			})
			...
			break
		case 'textChart':
			settings[chart].keywords = topics.filter(d => d.name === self.settings.yVar)[0].list.split(", ");
			const document = self.settings.data.filter(function(d){
				return String(d.id) == String(self.settings.selected)})[0].body.split(" ");

			settings[chart].text = document;
			break
	}
}
...
TextView.prototype.setColor = function() {
	...
	if(self.settings.colorVar == 'none') {
		...
		if(self.settings.colorVar == 'turkish' || self.settings.colorVar == 'kurdish'|| self.settings.colorVar == 'civilians') var colorScale = d3.scale.threshold().domain([0.5]).range(["rgb(100, 100, 100)","red"])
		else var colorScale = Number(self.settings.data[0][self.settings.colorVar]) != self.settings.data[0][self.settings.colorVar] ? d3.scale.category10(): d3.scale.linear().range(self.settings.colorRange).domain(colorDomain)
